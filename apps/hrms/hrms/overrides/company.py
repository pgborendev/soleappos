...
def validate_default_accounts(doc, method=None):
	...
	if doc.default_cash_account:
		if get_account_currency(doc.default_cash_account) != doc.default_currency:
			frappe.throw(
				_("Default Cash Account currency must be same as company's default currency. Please select another account.")
			)
