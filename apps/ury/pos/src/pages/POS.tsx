...
export default function POS() {
  ...
  const QuickFilterButton = ({ filter, icon: Icon, label }: {
    ...
    <button
      ...
      className={cn(
        ...
        quickFilter === filter ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200',
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          <p className="text-xl font-semibold text-red-600 mb-2">{t('failed_load_pos')}</p>
  ...
  return (
    <div className="flex flex-1 overflow-hidden">
        ...
        <div className="p-3 sm:p-4 bg-white border-b border-gray-200 flex-shrink-0">
          <div className="max-w-screen-xl mx-auto">
            <div className="flex items-center gap-2 overflow-x-auto overflow-y-hidden">
              <QuickFilterButton filter="all" icon={Star} label={t('all')} />
              <QuickFilterButton filter="special" icon={TrendingUp} label={t('special_items')} />
            </div>
          </div>
        </div>
      ...
      <OrderPanel isMobileOpen={mobileCartOpen} onMobileClose={() => setMobileCartOpen(false)} />
